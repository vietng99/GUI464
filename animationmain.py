...
def update_plot():
    ...
    #animation
    # Use FuncAnimation to dynamically update the plot
    anim = FuncAnimation(fig, update_plot, fargs=(ax, fig, canvas, config_storage), interval=100)
    preview_window.after(1000, anim.event_source.start)  # Start the animation with a delay for window stabilization
...
def open_preview_display():
    ...
    # Initialize Matplotlib figure and axes for the PWM graph
...
def generate_waveform():
    ...
    frequency= config_storage["frequency"]
    duty_cycle= config_storage["duty_cycle"]
    t = np.linspace(0, 1, 1000) *(1/frequency)  # Time axis
    y = np.where((t % (1/frequency) ) < (1/frequency * (duty_cycle/100)), 1, 0)  # Example square wave pulse 
    return t, y
